...
int main_d3d11_manual_vsync2()
{
	...
	while (1)
	{
		...
		static Uint64 last_dfdf_time;

		while (true)
		{
			DWM_TIMING_INFO timing_info;
			timing_info.cbSize = sizeof(DWM_TIMING_INFO);
			HRESULT bleh = DwmGetCompositionTimingInfo(NULL, &timing_info);
			if (FAILED(bleh))
			{
				printf("wtf DwmGetCOmsdf failed\n");
			}
			//double sync_ms = (double)frame_stats.SyncQPCTime.QuadPart / (double)SDL_GetPerformanceFrequency() * 1000.0;
			printf("%d frame stats cRefresh %llu dxRefresh %lu\n", frame_counter, timing_info.cBuffersEmpty, timing_info.cDXPresentSubmitted);

			if (last_present_refresh_count != timing_info.cFrame)
			{
				//last_present_count = timing_info.cFrame;
				last_present_refresh_count = timing_info.cFrame;
				double elapsed = get_elapsed_ms(last_dfdf_time);
				last_dfdf_time = SDL_GetPerformanceCounter();
				printf("changed to %llu, %lf\n", timing_info.cFrame, elapsed);
				break;
			}
